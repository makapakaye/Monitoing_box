...
BME680::BME680(int8_t cspin, SPIClass *theSPI)
    : _meas_start(0), _meas_period(0)
{
  _spidev = new SPIDevice(cspin, 1000000, SPI_BITORDER_MSBFIRST,
                                   SPI_MODE0, theSPI);
}
 ...
 *          SPI Clock
 */
BME680::BME680(int8_t cspin, int8_t mosipin, int8_t misopin,
                                 int8_t sckpin)
    : _meas_start(0), _meas_period(0)
{
  _spidev = new SPIDevice(cspin, sckpin, misopin, mosipin, 1000000,
                                   SPI_BITORDER_MSBFIRST, SPI_MODE0);
}

/**
 * @brief Set the SPI pins.
 * 
 * @param sda The pin number for the Serial Data (SDA) line.
 * @param scl The pin number for the Serial Clock (SCL) line.
 */
void BME680::setSPIPins(int sda, int scl)
{
 ...
 */
bool BME680::begin(uint8_t addr, bool initSettings)
{
  int8_t rslt;
  ...
  { // i2c
    ...
    _i2cdev = new I2CDevice(addr, _wire);
    if (!_i2cdev->begin())
    {
      return false;
    }
 ...
 */
bool BME680::performReading(void) { return endReading(); }
 ...
 * BME680::reading_not_started. Does not block.
...
static int8_t spi_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len,
                       void *intf_ptr)
{
  SPIDevice *_dev = (SPIDevice *)intf_ptr;
